                  ...
                  (with-current-buffer mb
                    ...
                    (when (<= (minibuffer-depth) 1)
                      (when-let* ((cand (fzfa--frontend-candidate)))
                        ...
                        ;; changes, not just its text.  The grep and
                        ;; location previews derive their overlays from
                        ;; the candidate's own `completions-common-part'
                        ;; faces, and those grow as the query lengthens
                        ;; even while the selection stays on one line
                        ;; (same text) — so a plain `equal' would freeze
                        ;; the overlay on a partial match ("emba" of
                        ;; "embark").  Hence `equal-including-properties'.
                        ;; And store a COPY: the sync scorer
                        ;; (`fzf-native-score-all', used by swiper)
                        ;; re-highlights candidates IN PLACE and hands
                        ;; back the same string objects, so keeping a
                        ;; reference would compare the object to its own
                        ;; mutated self and never see the change.  A
                        ;; snapshot does.
