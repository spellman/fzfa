...
Grep-style previews (~fzfa-grep~ — used by ~fzfa-rg~, ~fzfa-ag~,
~fzfa-grep~, ~fzfa-ugrep~, git/hg grep, …) highlight the match the way
~consult-ripgrep~ does: a full-width ~fzfa-preview-line~ overlay marks
the previewed line, ~fzfa-preview-match~ overlays mark the columns fzf
scored against (read from the candidate's own ~completions-common-part~
faces, so they line up with the highlights the minibuffer already
shows), and point lands on the first match.  Because the preview window
