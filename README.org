...
| ~fzfa-match-highlight~         | ~global~                 | Remap the minibuffer match face to ~fzfa-match~ for contrast (see note).       |
...
Because some themes render that face with low contrast, fzfa can
buffer-locally recolor matched runs to the ~fzfa-match~ face.
~fzfa-match-highlight~ controls the scope: ~global~ (the default) installs
the recolor via ~minibuffer-setup-hook~ so every completion UI that applies
~completions-common-part~ (fussy, orderless, flex, etc.) shows the same
match color as fzfa; ~t~ limits the recolor to fzfa's own sessions; ~nil~
leaves ~completions-common-part~ unchanged.  ~fzfa-match~ defaults to the
same ~match~ face the preview match overlay (~fzfa-preview-match~) inherits,
so minibuffer and preview matches share the theme's match color.  Only the
foreground and weight are applied — not the background — so a match shows
as colored text rather than a filled block and the selection highlight
shows through.  Customize the ~fzfa-match~ face to change the look.
