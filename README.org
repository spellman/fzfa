...
| ~project~   | ~fzfa-project~   | none (built-in)                                     | ~fzfa-project-find-file~, ~fzfa-project-find-dir~, ~fzfa-project-buffer~, ~fzfa-project-recentf~, ~fzfa-project-switch-project~ — candidate sets driven by ~project-files~ / ~project-buffers~ / ~fzfa-project-roots-dirs~                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| ~fzfa-preview-cursor~          | ~box~                    | Cursor on the previewed line in grep-style previews; nil leaves it alone.     |
...
| ~fzfa-batch-highlight~         | 200                      | Sync-path counterpart of ~fzfa-highlight~ (see note below).                   |
| ~fzfa-match-highlight~         | ~global~                 | Recolor minibuffer matches to ~fzfa-match~ for contrast (see note).           |
...
| ~fzfa-location~ | Open the SOURCE (file or buffer) carried on the candidate at LINE, highlighting the match. |
...
from the candidate's own ~completions-common-part~ faces, so they line up
with the highlights the minibuffer already shows), and point lands on the
first match.  Because the preview window is never the selected window, its
cursor would otherwise render as a dim hollow box; ~fzfa-preview-cursor~
(default ~box~) is applied buffer-locally to
~cursor-in-non-selected-windows~ for the duration of the preview so the
matched line carries a solid cursor.  All overlays and the cursor override
are torn down when the selection moves and when the minibuffer closes, so
the buffer you ultimately visit is left clean.  Set
~fzfa-preview-highlight~ to ~nil~ to preview without the line/match
overlays, or ~fzfa-preview-cursor~ to ~nil~ to leave the cursor alone.

The preview also refreshes when only the match highlight changes: as the
query lengthens the selected line's text stays the same while its matched
run grows, and the preview overlays follow it.
...
~fzfa-batch-highlight~ is the same cap for the synchronous scoring path —
~fzfa-swiper~, ~fzfa-M-x~, ~fzfa-buffer~ and the other single-pass
commands — and defaults to the same 200.  Each keystroke re-highlights
only the top N, so a candidate that drops out of the top N keeps the faces
the previous, shorter query gave it; set the cap low and a buffer with many
matches shows some lines highlighted through the whole query and others
frozen at a prefix of it.  Backspacing to an empty query clears the faces
outright rather than leaving the last query's highlights on screen.

The minibuffer's match color is the ~completions-common-part~ face, which
some themes render with low contrast.  ~fzfa-match-highlight~ recolors
matched runs to the foreground and weight of the ~fzfa-match~ face instead:
~global~ (the default) installs the recolor via ~minibuffer-setup-hook~, so
every completion UI that applies ~completions-common-part~ (fussy,
orderless, flex, …) shows the same match color as fzfa; ~t~ limits it to
fzfa's own sessions; ~nil~ leaves the face untouched.  ~fzfa-match~
...
the theme's match color.  Only foreground and weight are applied — not the
background — so a match reads as colored text rather than a filled block
and the selection highlight shows through.
